...
The chapter reader lets users save prepared words directly from its chapter list. Reprocessing a chapter clears that chapter's old vocabulary links only after extraction succeeds and before the refreshed list is saved; entries still linked to other chapters are retained, while unlinked entries are removed.

Reading completion is stored independently from vocabulary processing in `reading_progress.completed`; book progress is the percentage of chapters marked read. Book cover photos are stored in the private `book-covers` Storage bucket under an owner/book path, with short-lived signed URLs and authenticated Storage policies. When a custom cover is present, its title and language overlay are hidden so the photo is unobstructed. Deleting a book or account also removes its cover image.
